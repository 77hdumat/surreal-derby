...
브라우저에서 바로 실행되는 3D 엽기 경마 레이싱. 말 9종 × 기수 6명 조합을 골라 3.3km 굴곡 서킷(코너 35개)에서 최대 4인이 2바퀴 대결한다.
...
| ← → / A D | 조향 |
| Shift | 드리프트 (게이지 충전) |
| Space | 부스터 (최대 2개 저장, 3초. 사용 중엔 다음 것 안 나감) |
| 드리프트 직후 ↑ | 순간부스터 (톡톡이 연속 가능) |
| GO 직전 ↑ | 출발 부스터 |

드리프트 중 벽·다른 말에 부딪히면 그 드리프트로 모으던 게이지를 잃는다. 부스트 중엔 말마다 고유 모션이 나온다
