...
- [ ] vitest 설치, `npm test` 스크립트
- [ ] 테스트: 여러 (s,lat) 에 대해 getPoint → project 왕복 오차 < 1e-3 (s 는 wrap 비교)
- [ ] 구현: 직선 구간(|x| ≤ straight/2)은 z 부호로 앞/뒤 직선 선택 후 s 계산, 그 외는 반원 중심 (±straight/2, 0) 기준 atan2. 후보 두 직선/두 호 중 |lat| 최소 선택
- [ ] 커밋
...
```
- [ ] 테스트: 직진 throttle → speed 가 maxSpeed 초과 안 함 / 드리프트(steer=1, drift) 1초 → gauge > 0, slip ≠ 0 / gauge=1 + boost → boostT=BOOST_DURATION, gauge=0, maxSpeed 상승 / 벽 밖 위치 → lat 클램프 + 'wall' 이벤트 / progress 랩 언랩
- [ ] 구현 (spec 물리 절 수식 그대로)
- [ ] 커밋
...
- [ ] 테스트: balance 기수는 배수 1 / speed 기수 maxSpeed > balance
- [ ] 커밋
...
```
- [ ] 테스트: 4 CPU setup → startCountdown → step 3s → RACING / 충분히 step → 전원 finished, ranking 길이 4, results 시간 오름차순
- [ ] 커밋
...
- [ ] 테스트: encodeSnap → 배열 필드 순서/개수 고정, 소수 2자리 반올림
- [ ] 커밋
...
- [ ] 브라우저: 솔로 3바퀴 완주, 탭 2개 호스트/게스트 연결 후 레이스
